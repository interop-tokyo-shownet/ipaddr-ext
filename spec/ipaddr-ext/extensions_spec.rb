...
RSpec.describe IPAddrExt::Extensions do
  context "#broadcast" do
    it "ipv4" do
      expect(IPAddr.new("192.168.1.2/8").broadcast.to_s).to eq "192.255.255.255"
      expect(IPAddr.new("192.168.1.2/16").broadcast.to_s).to eq "192.168.255.255"
      expect(IPAddr.new("192.168.1.2/24").broadcast.to_s).to eq "192.168.1.255"
      expect(IPAddr.new("192.168.1.2/32").broadcast.to_s).to eq "192.168.1.2"
    end
  ...
  context "#==" do
    it "ipv4" do
      expect(IPAddr.new("192.168.1.2") == IPAddr.new("192.168.1.2")).to eq true
      expect(IPAddr.new("192.168.1.1") == IPAddr.new("192.168.1.2")).to eq false

      expect(IPAddr.new("192.168.1.0/24") == IPAddr.new("192.168.1.0/24")).to eq true
      expect(IPAddr.new("192.168.1.0/24") == IPAddr.new("192.168.2.0/24")).to eq false
    ...
    it "ipv6" do
      expect(IPAddr.new("3ffe:505:2::1") == IPAddr.new("3ffe:505:2::1")).to eq true
      expect(IPAddr.new("3ffe:505:2::1") == IPAddr.new("3ffe:505:2::2")).to eq false

      expect(IPAddr.new("3ffe:505:2::/64") == IPAddr.new("3ffe:505:2::/64")).to eq true
